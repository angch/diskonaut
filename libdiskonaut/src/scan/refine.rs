...
//!
//! [`FileTree::apply_found`]: crate::FileTree::apply_found
...
struct Pending {
    ...
    names: Vec<u8>,
    ends: Vec<u32>,
    sizes: Vec<crate::model::Sizes>,
}

impl Pending {
    #[cfg_attr(not(target_os = "linux"), allow(dead_code))]
    fn iter(&self) -> impl Iterator<Item = (&OsStr, crate::model::Sizes)> {
...
impl SmallFiles {
    ...
    pub fn note(&mut self, directory: &DirEntries) {
        ...
        for &index in &directory.later {
            ...
            pending.sizes.push(crate::model::Sizes::of(
                entry.meta.size,
                entry.meta.apparent,
            ));
        }
