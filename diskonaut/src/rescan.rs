...
use libdiskonaut::scan::refine::{Found, SmallFiles, refine};
    ...
    pub fn spawn(
        ...
        let _ = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                if depth > 0 && !libdiskonaut::scan::walk_would_enter(&root, &path, options) {
                    done(id, Outcome::NotWalked);
                    return;
                }
                ...
                if let Some((mut tree, failed, _, mut small)) = built {
                    ...
                    if refine_here {
                        ...
                        refine(
                            ...
                            libdiskonaut::scan::thread_count(options),
                            &Mutex::new(None),
                            &keep_going,
                            |found, _| {
