...
//!
//! The scan comes from `diskonaut-scan` and the tree, the squarify layout and deletion from
//! `libdiskonaut` — the same code the terminal viewer runs — so this crate is only a window and
//! some GDI drawing. `docs/features.md` lists what the terminal viewer has that this one does not
//! yet; none of it needs more than drawing and input here. It is deliberately
...
mod gui {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        ...
        IDYES, LoadCursorW, LoadIconW, MB_ICONERROR, MB_ICONWARNING, MB_OK, MB_YESNO, MSG,
        MessageBoxW, PostMessageW, PostQuitMessage, RegisterClassW, SW_SHOW, SetProcessDPIAware,
        SetWindowLongPtrW, SetWindowTextW, ShowWindow, TranslateMessage, WM_APP, WM_CREATE,
        WM_DESTROY, WM_KEYDOWN, WM_LBUTTONDOWN, WM_MOUSEMOVE, WM_PAINT, WM_RBUTTONDOWN, WM_SIZE,
    ...
    fn delete_selected(state: &mut AppState, hwnd: HWND) {
        ...
        let to_delete = FileToDelete::in_current_folder(
            tree,
            FileMetadata {
        ...
        if let Some(name) = libdiskonaut::delete::refused(::std::slice::from_ref(&to_delete)) {
            message(
                hwnd,
        ...
        // Only what actually left the disk comes off the tree, so the space freed is real.
        if let Err(error) = libdiskonaut::delete::remove(&to_delete) {
            message(
                hwnd,
                &format!(
    ...
    /// A message box over `hwnd`, captioned for deletion; returns which button closed it.
    fn message(hwnd: HWND, text: &str, style: u32) -> i32 {
        let text = wide(text);
        let caption = wide("Delete");
