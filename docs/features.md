...
| `preview` | `read`: what a file is from its first 64 KiB — empty, text (its first lines, made safe to draw), a PNG or JPEG, or binary, for which `describe_binary` gives its size and, on Linux, where its blocks are: `placement::describe` — extents, sparseness, shared blocks, and the physical disk (model, SSD or HDD) and offset, through a partition; the members of an LVM or md volume; a loop device's file. `describe_picture` from the header alone, and `decode_picture` within limits (40,000 pixels a side, 1 GiB). Only regular files are read, and on macOS never one that is only in iCloud. |
